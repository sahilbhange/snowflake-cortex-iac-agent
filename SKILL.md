...
- Adding account-level objects (resource monitors, network rules, external access integrations)
...
See `references/naming-conventions.md` for all object naming patterns, env suffix rules, and the NAME PROPOSAL format.
...
- `new-workload`, `new-role-user`, `account-objects`: NAME PROPOSAL gate first, then generate tfvars, run plans, wait for apply approval
...
Assistant: Routes to `$coco-iac-agent-new-workload`, presents NAME PROPOSAL table for MARKETING_READ + MARKETING_ROLE_TEST + MARKETING_WH_TEST + schemas, waits for approval, then generates tfvars entries and runs plans for roles -> warehouses -> schemas stacks in order.

### Example 2: Add a user
User: `$coco-iac-agent add user jsmith, email jsmith@company.com, ANALYST_ROLE, prod`
Assistant: Routes to `$coco-iac-agent-new-role-user`, presents NAME PROPOSAL (login: jsmith, object: JSMITH), on approval generates create_users.tfvars entry, runs plan for account_governance/users.
...
Assistant: Routes to `$coco-iac-agent-git-push`. Detects changed configs, generates branch `feat/test-add-marketing-workload`, commit message `feat(configs): onboard MARKETING squad — role, warehouse, schema in test`, outputs complete git command block + PR body for user to copy-paste and run.
...
Assistant: Routes to `$coco-iac-agent-account-objects`. Presents NAME PROPOSAL (PYPI_NETWORK_RULE, PYPI_ACCESS_INTEGRATION). On approval, verifies ADMIN_DB.GOVERNANCE schema exists, adds entries to `create_network_rules.tfvars` and `create_external_access_integrations.tfvars`. Runs plans in dependency order. Outputs apply commands.
