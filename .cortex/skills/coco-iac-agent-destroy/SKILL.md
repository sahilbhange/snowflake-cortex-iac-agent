...
- For roles: check if any user has this role, or any other role references it in `granted_roles`
...
- stages → external_access_integrations → network_rules → schemas → resource_monitors → storage_integrations → warehouses → users → databases → roles
